    ...
    <a-card :bordered="false">
      <a-row :gutter="24">
        ...
        <a-col :span="24">
                ...
                <!-- <a-col :md="6" :sm="24">
                  <a-form-item label="单位">
                    <a-tree-select v-model="queryParam.deptId" style="width: 100%"
                      ...
                      placeholder="请选择" :replaceFields="replaceFields" tree-default-expand-all>
                    </a-tree-select>
                  </a-form-item>
                </a-col>
          ...
          <!-- 增加修改 -->
          <!-- 创建/编辑用户,单独封装了组件 -->
          <create-form ref="createForm" @adminList="getAdminList" :deptOptions="deptOptions" @ok="getList" />
          <!-- 修改密码抽屉 -->
          ...
          <a-table :loading="loading" :size="tableSize" rowKey="userId" :columns="columns" :data-source="list"
            ...
            <span slot="loginStatus" slot-scope="text, record">
              <a-popconfirm v-if="record.loginIp" ok-text="是" cancel-text="否" @confirm="confirmHandleIpStatus(record)">
                <span slot="title">确认<b>{{ record.loginStatus === '0' ? '启用' : '停用' }}</b>{{ record.loginIp }}
                  IP白名单地址吗?</span>
                <a-switch checked-children="开" un-checked-children="关" :checked="record.loginStatus == 1" />
              </a-popconfirm>
            </span>
            ...
            <span slot="loginIpNew" slot-scope="text, record">
              <span v-for="(ip, index) in parseIpList(record.loginIpNew)" style="color: #F56C6C;">{{ ip }}
                <span style="color: #ccc;" v-if="index < parseIpList(record.loginIpNew).length - 1">| </span>
              </span>
            </span>
...
<script>
import { listUser, delUser, changeUserStatus, getUser, changeLoginStatus } from '@/api/system/user'
...
export default {
  ...
  methods: {
    ...
    /* 白名单状态修改 */
    confirmHandleIpStatus(row) {
      const text = row.loginStatus === '1' ? '启用' : '关闭'
      row.loginStatus = row.loginStatus === '0' ? '1' : '0'
      changeLoginStatus(row.userId, row.loginStatus)
        .then(() => {
          this.$message.success(text + '成功', 3)
        })
        .catch(function () {
