<template>
  <div>
    <create-data-form ref="createDataForm" :statusOptions="dict.type['sys_normal_disable']" :dictType="dictType"
      @ok="getList" />
    <a-card :bordered="false" :style="{ background: '#f2f4f5' }">
      <div class="table-operations">
        <a-button type="primary" size="small" @click="$refs.createDataForm.handleAdd()" v-hasPermi="['system:dict:add']"
          ghost>
          <a-icon type="plus" />新增
        </a-button>
        <a-button type="danger" :disabled="multiple" @click="handleDelete" size="small"
          v-hasPermi="['system:dict:remove']" ghost>
          <a-icon type="delete" />删除
        </a-button>
        <table-setting :style="{ float: 'right' }" :table-size.sync="tableSize" v-model="columns"
          :refresh-loading="loading" @refresh="getList" />
      </div>
      <a-table :loading="loading" rowKey="dictCode" :size="tableSize" :columns="columns" :data-source="list"
        :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :scroll="{ y: 300 }"
        :pagination="false">
