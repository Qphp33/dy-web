...
export function parseDate(time, pattern) {
  ...
  const format = pattern || '{y}-{m}-{d}' // 修改为只返回年月日
  let date
  if (typeof time === 'object') {
    date = time
  } else {
  ...
  const timeStr = format.replace(/{(y|m|d)+}/g, (result, key) => { // 只匹配 y, m, d
    let value = formatObj[key]
    if (result.length > 0 && value < 10) {
      value = '0' + value
    }
...
export const isValidData = (data) => {
  if (Array.isArray(data)) {
    return data.length > 0; // 如果是数组，检查是否为空
  }
  if (data && typeof data === 'object') {
    return Object.keys(data).length > 0; // 如果是对象，检查是否有键值对
  }
