...
**Goal:** Optimize the Address module into a Douyin-inspired light UI without changing business behavior.

**Architecture:** Keep the existing Vue 2 single-file components and Ant Design Vue controls. Change template wrappers and scoped Less/CSS classes only where needed, preserving API calls, pagination, modals, and row actions. Keep the project’s existing light theme and use cleaner buttons, card hierarchy, and Douyin admin information hierarchy rather than a dark theme or heavy hero section.

**Tech Stack:** Vue 2, Ant Design Vue 1.x, Vue CLI 4, scoped styles in `.vue` files.

---

### Task 1: Address list page light visual refresh
...
Keep `.table-operations` as a compact action area that still calls `$refs.CreateForm.handleAdd()` and keeps `<table-setting>`.
...
Keep all columns and slots, but change scoped CSS so the table uses rounded light headers, soft shadows, and Douyin-style pills. Do not change row background on hover.
...
Keep the `.address` wrapper light blue using the project’s existing background palette.

- [ ] **Step 2: Restyle region card and comments**

Keep the current left image/right comments layout, using the project’s existing light card language.

- [ ] **Step 3: Restyle filter bar and video cards**

Update `.video-list__sort-buttons` and `.video-list__video-card` to use white panels, blue primary controls, subtle red/cyan accents, and compact information pills.
